...
pub fn get_settings(conn: &Connection) -> Result<ExternalFolderSettings, String> {
    let result = conn.query_row(
        "SELECT enabled, account_id, account_email, system, connected_at
...
pub fn save_connection(
    ...
    conn.execute(
        "INSERT INTO external_folder_settings
            (id, enabled, account_id, account_email, system,
        ...
        rusqlite::params![account_id, account_email, system, enc_password, enc_nonce],
    )
...
mod tests {
    ...
    fn save_connection_round_trip() {
        let conn = open_test_db();
        let settings =
            save_connection(&conn, 2300, "contato@example.com", 1, b"cipher", b"nonce").unwrap();
        assert!(settings.enabled);
    ...
    fn set_enabled_toggles() {
        let conn = open_test_db();
        save_connection(&conn, 1, "a@b.com", 1, b"c", b"n").unwrap();
